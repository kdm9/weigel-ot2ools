...
#@NB: modify this script to:
# Track a source and destination plate name
# Instead of keeping just platename -> well -> (DNA vol, H2O vol) you need to
#     also keep track of the name of the source plate for each dest plate well.
# (in opentron part):
#   keep track of all known source plates, and lay them out in order
#   Make dest plates (check they fit!!!)
#   For each dest plate, run the same logic as before.
Well = namedtuple("Well", ["source_plate", "dest_plate", "well", "water_vol", "stock_vol"])
...
def generate_protocols(wells, outdir, stock_pipette, min_volume, max_volume):
    ...
    for well in tqdm(wells):
        ...
        plates[well.plate][well.well] = {
            ...
            "source_plate": well.source_plate,
        }
...
def read_instructions(args):
    ...
    with open(args.instructions) as fh:
        for row in csv.DictReader(fh, dialect=dialect):
            try:
                yield Well(
                        source_plate=row[args.source_plate_column],
                        dest_plate=row[args.plate_column],
                ...
                print(f"you asked for: {args.source_plate_column}, {args.plate_column}, {args.well_column}, {args.water_column}, {args.stock_column}", file=stderr)
...
def main(argv=None):
    ...
    ap.add_argument("--spc", "--source-plate-column", dest="source_plate_column", type=str,
    ...
    if args.source_plate_column is None:
        args.source_plate_column = args.plate_column
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    source_plate_pos = {}
    srcs = {}
    for plate_name, wells in plates.items():
        for well in wells.values()
            if well["source_plate"] not in source_plate_pos:
                source_plate_name = well["source_plate"]
                source_plate_pos[source_plate_name] = pos
                srcs[source_plate_name] = protocol.load_labware('axygen_96_wellplate_200ul', pos, label=f"STOCK {source_plate_name}")
                pos += 1
    dests = {}
    for dest_plate_name in plates:
        dests[dest_plate_name] = protocol.load_labware('axygen_96_wellplate_200ul', pos+1, label=f"DILUTED {plate_name}")
        pos += 1
    ...
    for plate_name, wellvols in plates.items():
        ...
        for well_pos, well in wellvols.items():
            src = srcs[well["source_plate"]]
            if well["stock"] < 0.01:
                continue
            pipette.pick_up_tip()
            dispense(src[well_pos], dst[well_pos], vols["stock"], pipette)
            pipette.drop_tip()
"""
